...
function isPrime(value) {
  if (!isPrime.answers) { // if there is no cache. Using answers property as cache here
    isPrime.answers = {}; // if no cache, create a cache. Only occurs on the FIRST CALL to the function
  }
  // check if the passed in value has already been cached
  ...
  for (var i = 2; i < value; i++) { // grab the value, divide it with every number from 2 to the value-1 itself
    if (value % i === 0) { // if there is remainder (9 % 2 = 1), increment i (9 % 3 = 0, not prime, so break)
      prime = false; // if you pass 5, 5 % 2 = 1, 5%3=2, 5%4=1, 5%5-> it never gets there
      break;
    }
  }
...
assert(isPrime(7), console.log('if no assert error: value is prime, if assert error: value is not prime'));
assert(isPrime.answers[7], console.log('if no assert error: The answer was cached, if assert error: value was not cached'));